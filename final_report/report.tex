...
% Twitter
Twitter provides an official API\footnote{\url{https://developer.twitter.com/en/docs/twitter-api}} for accessing tweets, but access to this requires an approval process. Instead, the open source Python library snscrape\footnote{\url{https://github.com/JustAnotherArchivist/snscrape}} was used to collect 240 tweets for each day in the range January 2017 to October 2021, filtered by searching for the word “Ethereum”. The popularity content for each tweet was calculated by summing together the number of retweets, likes, and replies it received. This popularity value was then normalized by dividing it by the highest popularity value of all collected tweets.

% Reddit
Reddit is divided into smaller sub-communities relating to specific themes called “subreddits”. The subreddits "EtherMining", "Cryptocurrency", "Cryptocurrencies", "CryptoMarkets", "EthTrader", "Ethereum", and "Bitcoin" were selected as a source of data because of their relevancy. Comments posted on submissions on each of these subreddits were scraped using the library psaw\footnote{\url{https://github.com/dmarx/psaw}}, which itself pulls data from the service Pushshift\footnote{\url{https://search.pushshift.io/reddit/}} . The popularity value of each comment was determined by its “karma” value, a measure of how much positive feedback it has received from other users of the site. Up to 240 comments were scraped per subreddit per day in the same time range as Twitter, for a maximum of 1680 per day if that many comments had actually been posted.
