...
The K-nearest Neighbours (kNN) is a supervised machine learning algorithm used to solve classification and regression problems. KNN model determines the class of the target variable by looking at its nearest neighbours which means the nearest training data points. K is the number of such data points that has to be considered in the algorithm. Since the project deals with the classification problem, the class label assigned to the majority of those neighbours has been considered to be the class for the target value.

\subsection{Support Vector Machine}

Support Vector Machine (SVM) is a classifier similar to logistic regression, the prediction uses $sign(\theta^Tx)$ and the output is mapped to $+1$ and $-1$. 
